...
then
    ...
    read setupInt2Ans
        if [ "$setupInt2Ans" = "y" ]
        then
            echo "Please enter the WAG IPv4 endpoint address"
            read wagIpv4
            echo $wagIpv4 >> load_var.woc
        clear
        echo "The following parameters have been configured:"
        ipv4_wan=$(ip -6 addr show ens192 scope global | egrep -v dynamic | awk '$1 == "inet" {print $2}' | awk '{print substr($1, 1, length($1)-3)}')
        echo "-------------------------------------------------------------------------"
        echo "The local IPv4 address for the dataplane tunnel is:"
        echo "$ipv4_wan"
        echo "-------------------------------------------------------------------------"
        echo "The WAG IPv4 endpoint is:"
        echo "$wagIpv4"
        echo "-------------------------------------------------------------------------"
        ...
        if [[ $enableService != "y" ]]; then
            ...
            #firewall-cmd --permanent --zone=trusted --add-source=$wagIpv4
            firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -p gre -j ACCEPT
            firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -p icmpv6 -s $wagIpv4 -j ACCEPT
